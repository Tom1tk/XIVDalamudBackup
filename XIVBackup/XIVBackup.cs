...
public unsafe class XIVBackup : IDalamudPlugin
{
    public string Name => "XIV Dalamud Backup";
    ...
    internal GitService gitService;
    internal static XIVBackup P;

    public XIVBackup(IDalamudPluginInterface pluginInterface)
    {
        P = this;
        ...
        windowSystem.AddWindow(restoreWindow);
        Svc.PluginInterface.UiBuilder.Draw += windowSystem.Draw;
        DoBackup();
        
        // Auto-sync on startup if enabled
        if (config.EnableGitHubSync && config.AutoSyncOnStartup)
        {
            _ = Task.Run(async () => await SyncToGitHubAsync());
        }
        ...
        Svc.Commands.AddHandler("/xivbackup-push", new Dalamud.Game.Command.CommandInfo(delegate {
            _ = Task.Run(async () => await SyncToGitHubAsync());
        })
        {
            HelpMessage = "push backup to GitHub"
        });
        Svc.Commands.AddHandler("/xivbackup-restore", new Dalamud.Game.Command.CommandInfo(delegate {
            restoreWindow.IsOpen = true;
        })
        {
            HelpMessage = "restore from GitHub"
        });
    ...
    public void Dispose()
    {
        ...
        Svc.Commands.RemoveHandler("/xivbackup-push");
        Svc.Commands.RemoveHandler("/xivbackup-restore");
        config = null;
        P = null;
    }
    ...
    {
        return config.BackupPath != string.Empty ? config.BackupPath : Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "XIVDalamudBackup");
    }
    ...
    internal void DoBackupInternal()
    {
        ...
        {
            temp = Path.Combine(config.TempPath, $"XIVDalamudBackup-{stamp}");
        }
        else
        {
            temp = Path.Combine(Path.GetTempPath(), $"XIVDalamudBackup-{stamp}");
        }
        ...
        var thread = new Thread(() =>
        {
            ...
            try
            {
                ...
                {
                    if (!x.Contains("XIVDalamudBackup-"))
                    {
                ...
                if (pluginSuccess && gameSuccess && config.EnableGitHubSync)
                {
                    new TickScheduler(() => _ = Task.Run(async () => await SyncToGitHubAsync()));
                }
            }
            catch(Exception ex)
            {
    ...
    /// <summary>
    /// Sync current configs to GitHub
    /// </summary>
    internal async Task<bool> SyncToGitHubAsync()
    {
        try
        {
            if (!config.EnableGitHubSync)
            {
                PluginLog.Debug("GitHub sync is disabled");
                return false;
            }

            if (string.IsNullOrEmpty(config.GitHubToken))
            {
                PluginLog.Warning("GitHub token not configured");
                Notify.Warning("Please configure GitHub settings first");
                return false;
            }

            if (!gitService.IsRepositoryInitialized())
            {
                PluginLog.Warning("Repository not initialized");
                Notify.Warning("Please initialize GitHub repository first");
                return false;
            }

            PluginLog.Information("Starting GitHub sync...");

            // Copy files to repo
            await gitService.CopyGameFilesToRepo();

            // Commit and push
            var message = $"Backup: {DateTimeOffset.Now:yyyy-MM-dd HH:mm:ss}";
            var success = await gitService.CommitAndPushAsync(message);

            if (success)
            {
                config.LastCommitSha = await gitService.GetCurrentCommitShaAsync();
                Svc.PluginInterface.SavePluginConfig(config);
                Notify.Success("Synced to GitHub successfully!");
            }
            else
            {
                Notify.Error("Failed to sync to GitHub");
            }
        ...
        {
            PluginLog.Error($"GitHub sync failed: {ex.Message}\n{ex.StackTrace ?? ""}");
            Notify.Error("Failed to sync to GitHub: " + ex.Message);
            return false;
        }
    }

    /// <summary>
    /// Restore configs from GitHub
    /// </summary>
    internal async Task<bool> RestoreFromGitHubAsync(string commitSha)
    {
        try
        {
            if (!gitService.IsRepositoryInitialized())
            {
                Notify.Error("Repository not initialized");
                return false;
            }

            PluginLog.Information($"Restoring from commit {commitSha}...");

            // Checkout the selected commit
            var checkoutSuccess = await gitService.CheckoutCommitAsync(commitSha);
            if (!checkoutSuccess)
            {
                Notify.Error("Failed to checkout commit");
                return false;
            }

            // Restore files
            await gitService.RestoreFilesFromRepo();

            Notify.Success("Restored from GitHub! Please restart the game to apply changes.");
            return true;
        }
        catch (Exception ex)
        {
            PluginLog.Error($"Restore failed: {ex.Message}\n{ex.StackTrace ?? ""}");
            Notify.Error("Failed to restore from GitHub: " + ex.Message);
            return false;
        }
    }
}
