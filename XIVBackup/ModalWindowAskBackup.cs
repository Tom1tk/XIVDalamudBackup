...
{
    public ModalWindowAskBackup() : base("XIV Dalamud Backup - another instance is already running", ImGuiWindowFlags.NoCollapse | ImGuiWindowFlags.AlwaysAutoResize, true)
    {
        this.IsOpen = false;
    ...
    public override void Draw()
    {
        ...
        if (ImGui.Button($"Force stop old backup and create new backup"))
        {
            try
            {
                ...
                (ECommonsMain.Instance as XIVBackup).DoBackupInternal();
                this.IsOpen = false;
            }
            catch(Exception ex )
            {
